...
class ApparenceController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('slide1_image')) {
            ...
            Image::make($image)->resize(1920,850)->save('slides/'.$name_gen);
            $slide1Image = 'slides/'.$name_gen;
        ...
        if ($request->hasFile('image_atout')) {
            ...
            $name_gen = hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
            Image::make($image)->resize(2879,1363)->save('images/'.$name_gen);
            $imageAtout = 'images/'.$name_gen;
